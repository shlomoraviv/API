.class public Landroid/support/v17/leanback/transition/SlideKitkat;
.super Landroid/transition/Visibility;
.source "\ubcfc\ub968 \ucee8\ud2b8\ub864\uc774 \ud45c\uc2dc\ub429\ub2c8\ub2e4. \ub2eb\uc73c\ub824\uba74 \uc704\ub85c \uc2a4\uc640\uc774\ud504\ud569\ub2c8\ub2e4"


# annotations
.annotation system Ldalvik/annotation/MemberClasses;
    value = {
        Landroid/support/v17/leanback/transition/SlideKitkat$j;,
        Landroid/support/v17/leanback/transition/SlideKitkat$i;,
        Landroid/support/v17/leanback/transition/SlideKitkat$h;,
        Landroid/support/v17/leanback/transition/SlideKitkat$g;
    }
.end annotation


# static fields
.field public static final c:Landroid/animation/TimeInterpolator;

.field public static final d:Landroid/animation/TimeInterpolator;

.field public static final e:Landroid/support/v17/leanback/transition/SlideKitkat$g;

.field public static final f:Landroid/support/v17/leanback/transition/SlideKitkat$g;

.field public static final g:Landroid/support/v17/leanback/transition/SlideKitkat$g;

.field public static final h:Landroid/support/v17/leanback/transition/SlideKitkat$g;

.field public static final i:Landroid/support/v17/leanback/transition/SlideKitkat$g;

.field public static final j:Landroid/support/v17/leanback/transition/SlideKitkat$g;


# instance fields
.field public b:Landroid/support/v17/leanback/transition/SlideKitkat$g;


# direct methods
.method public static constructor <clinit>()V
    .locals 1

    new-instance v0, Landroid/view/animation/DecelerateInterpolator;

    invoke-direct {v0}, Landroid/view/animation/DecelerateInterpolator;-><init>()V

    sput-object v0, Landroid/support/v17/leanback/transition/SlideKitkat;->c:Landroid/animation/TimeInterpolator;

    new-instance v0, Landroid/view/animation/AccelerateInterpolator;

    invoke-direct {v0}, Landroid/view/animation/AccelerateInterpolator;-><init>()V

    sput-object v0, Landroid/support/v17/leanback/transition/SlideKitkat;->d:Landroid/animation/TimeInterpolator;

    new-instance v0, Landroid/support/v17/leanback/transition/SlideKitkat$a;

    invoke-direct {v0}, Landroid/support/v17/leanback/transition/SlideKitkat$a;-><init>()V

    sput-object v0, Landroid/support/v17/leanback/transition/SlideKitkat;->e:Landroid/support/v17/leanback/transition/SlideKitkat$g;

    new-instance v0, Landroid/support/v17/leanback/transition/SlideKitkat$b;

    invoke-direct {v0}, Landroid/support/v17/leanback/transition/SlideKitkat$b;-><init>()V

    sput-object v0, Landroid/support/v17/leanback/transition/SlideKitkat;->f:Landroid/support/v17/leanback/transition/SlideKitkat$g;

    new-instance v0, Landroid/support/v17/leanback/transition/SlideKitkat$c;

    invoke-direct {v0}, Landroid/support/v17/leanback/transition/SlideKitkat$c;-><init>()V

    sput-object v0, Landroid/support/v17/leanback/transition/SlideKitkat;->g:Landroid/support/v17/leanback/transition/SlideKitkat$g;

    new-instance v0, Landroid/support/v17/leanback/transition/SlideKitkat$d;

    invoke-direct {v0}, Landroid/support/v17/leanback/transition/SlideKitkat$d;-><init>()V

    sput-object v0, Landroid/support/v17/leanback/transition/SlideKitkat;->h:Landroid/support/v17/leanback/transition/SlideKitkat$g;

    new-instance v0, Landroid/support/v17/leanback/transition/SlideKitkat$e;

    invoke-direct {v0}, Landroid/support/v17/leanback/transition/SlideKitkat$e;-><init>()V

    sput-object v0, Landroid/support/v17/leanback/transition/SlideKitkat;->i:Landroid/support/v17/leanback/transition/SlideKitkat$g;

    new-instance v0, Landroid/support/v17/leanback/transition/SlideKitkat$f;

    invoke-direct {v0}, Landroid/support/v17/leanback/transition/SlideKitkat$f;-><init>()V

    sput-object v0, Landroid/support/v17/leanback/transition/SlideKitkat;->j:Landroid/support/v17/leanback/transition/SlideKitkat$g;

    return-void
.end method

.method public constructor <init>()V
    .locals 1

    invoke-direct {p0}, Landroid/transition/Visibility;-><init>()V

    const/16 v0, 0x50

    invoke-virtual {p0, v0}, Landroid/support/v17/leanback/transition/SlideKitkat;->a(I)V

    return-void
.end method

.method public constructor <init>(Landroid/content/Context;Landroid/util/AttributeSet;)V
    .locals 7

    invoke-direct {p0}, Landroid/transition/Visibility;-><init>()V

    sget-object v0, La/l6;->lbSlide:[I

    invoke-virtual {p1, p2, v0}, Landroid/content/Context;->obtainStyledAttributes(Landroid/util/AttributeSet;[I)Landroid/content/res/TypedArray;

    move-result-object v3

    sget v1, La/l6;->lbSlide_lb_slideEdge:I

    const/16 v0, 0x50

    invoke-virtual {v3, v1, v0}, Landroid/content/res/TypedArray;->getInt(II)I

    move-result v0

    invoke-virtual {p0, v0}, Landroid/support/v17/leanback/transition/SlideKitkat;->a(I)V

    sget v0, La/l6;->lbSlide_android_duration:I

    const/4 v6, -0x1

    invoke-virtual {v3, v0, v6}, Landroid/content/res/TypedArray;->getInt(II)I

    move-result v0

    int-to-long v1, v0

    const-wide/16 v4, 0x0

    cmp-long v0, v1, v4

    if-ltz v0, :cond_0

    invoke-virtual {p0, v1, v2}, Landroid/transition/Visibility;->setDuration(J)Landroid/transition/Transition;

    :cond_0
    sget v0, La/l6;->lbSlide_android_startDelay:I

    invoke-virtual {v3, v0, v6}, Landroid/content/res/TypedArray;->getInt(II)I

    move-result v0

    int-to-long v1, v0

    cmp-long v0, v1, v4

    if-lez v0, :cond_1

    invoke-virtual {p0, v1, v2}, Landroid/transition/Visibility;->setStartDelay(J)Landroid/transition/Transition;

    :cond_1
    sget v1, La/l6;->lbSlide_android_interpolator:I

    const/4 v0, 0x0

    invoke-virtual {v3, v1, v0}, Landroid/content/res/TypedArray;->getResourceId(II)I

    move-result v0

    if-lez v0, :cond_2

    invoke-static {p1, v0}, Landroid/view/animation/AnimationUtils;->loadInterpolator(Landroid/content/Context;I)Landroid/view/animation/Interpolator;

    move-result-object v0

    invoke-virtual {p0, v0}, Landroid/transition/Visibility;->setInterpolator(Landroid/animation/TimeInterpolator;)Landroid/transition/Transition;

    :cond_2
    invoke-virtual {v3}, Landroid/content/res/TypedArray;->recycle()V

    return-void
.end method


# virtual methods
.method public final a(Landroid/view/View;Landroid/util/Property;FFFLandroid/animation/TimeInterpolator;I)Landroid/animation/Animator;
    .locals 3
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "(",
            "Landroid/view/View;",
            "Landroid/util/Property<",
            "Landroid/view/View;",
            "Ljava/lang/Float;",
            ">;FFF",
            "Landroid/animation/TimeInterpolator;",
            "I)",
            "Landroid/animation/Animator;"
        }
    .end annotation

    sget v0, La/f6;->lb_slide_transition_value:I

    move-object p1, p1

    invoke-virtual {p1, v0}, Landroid/view/View;->getTag(I)Ljava/lang/Object;

    move-result-object v1

    check-cast v1, [F

    const/4 p0, 0x1

    const/4 v2, 0x0

    move-object p2, p2

    if-eqz v1, :cond_1

    sget-object v0, Landroid/view/View;->TRANSLATION_Y:Landroid/util/Property;

    if-ne v0, p2, :cond_0

    aget p3, v1, p0

    goto :goto_0

    :cond_0
    aget p3, v1, v2

    :goto_0
    sget v1, La/f6;->lb_slide_transition_value:I

    const/4 v0, 0x0

    invoke-virtual {p1, v1, v0}, Landroid/view/View;->setTag(ILjava/lang/Object;)V

    :cond_1
    const/4 v0, 0x2

    new-array v0, v0, [F

    aput p3, v0, v2

    move p4, p4

    aput p4, v0, p0

    invoke-static {p1, p2, v0}, Landroid/animation/ObjectAnimator;->ofFloat(Ljava/lang/Object;Landroid/util/Property;[F)Landroid/animation/ObjectAnimator;

    move-result-object v0

    new-instance p0, Landroid/support/v17/leanback/transition/SlideKitkat$j;

    move p3, p5

    move p5, p7

    invoke-direct/range {p0 .. p5}, Landroid/support/v17/leanback/transition/SlideKitkat$j;-><init>(Landroid/view/View;Landroid/util/Property;FFI)V

    invoke-virtual {v0, p0}, Landroid/animation/ObjectAnimator;->addListener(Landroid/animation/Animator$AnimatorListener;)V

    invoke-virtual {v0, p0}, Landroid/animation/ObjectAnimator;->addPauseListener(Landroid/animation/Animator$AnimatorPauseListener;)V

    invoke-virtual {v0, p6}, Landroid/animation/ObjectAnimator;->setInterpolator(Landroid/animation/TimeInterpolator;)V

    return-object v0
.end method

.method public a(I)V
    .locals 1

    const/4 v0, 0x3

    if-eq p1, v0, :cond_5

    const/4 v0, 0x5

    if-eq p1, v0, :cond_4

    const/16 v0, 0x30

    if-eq p1, v0, :cond_3

    const/16 v0, 0x50

    if-eq p1, v0, :cond_2

    const v0, 0x800003

    if-eq p1, v0, :cond_1

    const v0, 0x800005

    if-ne p1, v0, :cond_0

    sget-object v0, Landroid/support/v17/leanback/transition/SlideKitkat;->j:Landroid/support/v17/leanback/transition/SlideKitkat$g;

    goto :goto_0

    :cond_0
    new-instance p0, Ljava/lang/IllegalArgumentException;

    const-string v0, "Invalid slide direction"

    invoke-direct {p0, v0}, Ljava/lang/IllegalArgumentException;-><init>(Ljava/lang/String;)V

    throw p0

    :cond_1
    sget-object v0, Landroid/support/v17/leanback/transition/SlideKitkat;->i:Landroid/support/v17/leanback/transition/SlideKitkat$g;

    goto :goto_0

    :cond_2
    sget-object v0, Landroid/support/v17/leanback/transition/SlideKitkat;->h:Landroid/support/v17/leanback/transition/SlideKitkat$g;

    goto :goto_0

    :cond_3
    sget-object v0, Landroid/support/v17/leanback/transition/SlideKitkat;->f:Landroid/support/v17/leanback/transition/SlideKitkat$g;

    goto :goto_0

    :cond_4
    sget-object v0, Landroid/support/v17/leanback/transition/SlideKitkat;->g:Landroid/support/v17/leanback/transition/SlideKitkat$g;

    goto :goto_0

    :cond_5
    sget-object v0, Landroid/support/v17/leanback/transition/SlideKitkat;->e:Landroid/support/v17/leanback/transition/SlideKitkat$g;

    :goto_0
    iput-object v0, p0, Landroid/support/v17/leanback/transition/SlideKitkat;->b:Landroid/support/v17/leanback/transition/SlideKitkat$g;

    return-void
.end method

.method public onAppear(Landroid/view/ViewGroup;Landroid/transition/TransitionValues;ILandroid/transition/TransitionValues;I)Landroid/animation/Animator;
    .locals 3

    const/4 v0, 0x0

    if-eqz p4, :cond_0

    iget-object v2, p4, Landroid/transition/TransitionValues;->view:Landroid/view/View;

    goto :goto_0

    :cond_0
    move-object v2, v0

    :goto_0
    if-nez v2, :cond_1

    return-object v0

    :cond_1
    move-object v1, p0

    iget-object v0, v1, Landroid/support/v17/leanback/transition/SlideKitkat;->b:Landroid/support/v17/leanback/transition/SlideKitkat$g;

    invoke-interface {v0, v2}, Landroid/support/v17/leanback/transition/SlideKitkat$g;->b(Landroid/view/View;)F

    move-result p2

    iget-object v0, v1, Landroid/support/v17/leanback/transition/SlideKitkat;->b:Landroid/support/v17/leanback/transition/SlideKitkat$g;

    invoke-interface {v0, v2}, Landroid/support/v17/leanback/transition/SlideKitkat$g;->a(Landroid/view/View;)F

    move-result p1

    iget-object v0, v1, Landroid/support/v17/leanback/transition/SlideKitkat;->b:Landroid/support/v17/leanback/transition/SlideKitkat$g;

    invoke-interface {v0}, Landroid/support/v17/leanback/transition/SlideKitkat$g;->a()Landroid/util/Property;

    move-result-object p0

    sget-object p4, Landroid/support/v17/leanback/transition/SlideKitkat;->c:Landroid/animation/TimeInterpolator;

    const/4 p5, 0x0

    move p3, p2

    invoke-virtual/range {v1 .. v8}, Landroid/support/v17/leanback/transition/SlideKitkat;->a(Landroid/view/View;Landroid/util/Property;FFFLandroid/animation/TimeInterpolator;I)Landroid/animation/Animator;

    move-result-object v0

    return-object v0
.end method

.method public onDisappear(Landroid/view/ViewGroup;Landroid/transition/TransitionValues;ILandroid/transition/TransitionValues;I)Landroid/animation/Animator;
    .locals 3

    const/4 v0, 0x0

    if-eqz p2, :cond_0

    iget-object v2, p2, Landroid/transition/TransitionValues;->view:Landroid/view/View;

    goto :goto_0

    :cond_0
    move-object v2, v0

    :goto_0
    if-nez v2, :cond_1

    return-object v0

    :cond_1
    move-object v1, p0

    iget-object v0, v1, Landroid/support/v17/leanback/transition/SlideKitkat;->b:Landroid/support/v17/leanback/transition/SlideKitkat$g;

    invoke-interface {v0, v2}, Landroid/support/v17/leanback/transition/SlideKitkat$g;->b(Landroid/view/View;)F

    move-result p1

    iget-object v0, v1, Landroid/support/v17/leanback/transition/SlideKitkat;->b:Landroid/support/v17/leanback/transition/SlideKitkat$g;

    invoke-interface {v0, v2}, Landroid/support/v17/leanback/transition/SlideKitkat$g;->a(Landroid/view/View;)F

    move-result p2

    iget-object v0, v1, Landroid/support/v17/leanback/transition/SlideKitkat;->b:Landroid/support/v17/leanback/transition/SlideKitkat$g;

    invoke-interface {v0}, Landroid/support/v17/leanback/transition/SlideKitkat$g;->a()Landroid/util/Property;

    move-result-object p0

    sget-object p4, Landroid/support/v17/leanback/transition/SlideKitkat;->d:Landroid/animation/TimeInterpolator;

    const/4 p5, 0x4

    move p3, p1

    invoke-virtual/range {v1 .. v8}, Landroid/support/v17/leanback/transition/SlideKitkat;->a(Landroid/view/View;Landroid/util/Property;FFFLandroid/animation/TimeInterpolator;I)Landroid/animation/Animator;

    move-result-object v0

    return-object v0
.end method
