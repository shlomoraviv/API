.class public La/q8;
.super Ljava/lang/Object;
.source "\ubcfc\ub968 \ucee8\ud2b8\ub864\uc774 \ud45c\uc2dc\ub429\ub2c8\ub2e4. \ub2eb\uc73c\ub824\uba74 \uc704\ub85c \uc2a4\uc640\uc774\ud504\ud569\ub2c8\ub2e4"


# instance fields
.field public final a:Landroid/app/Notification$Builder;

.field public final b:La/p8$d;

.field public c:Landroid/widget/RemoteViews;

.field public d:Landroid/widget/RemoteViews;

.field public final e:Ljava/util/List;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "Ljava/util/List<",
            "Landroid/os/Bundle;",
            ">;"
        }
    .end annotation
.end field

.field public final f:Landroid/os/Bundle;

.field public g:I

.field public h:Landroid/widget/RemoteViews;


# direct methods
.method public constructor <init>(La/p8$d;)V
    .locals 10

    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    new-instance v0, Ljava/util/ArrayList;

    invoke-direct {v0}, Ljava/util/ArrayList;-><init>()V

    iput-object v0, p0, La/q8;->e:Ljava/util/List;

    new-instance v0, Landroid/os/Bundle;

    invoke-direct {v0}, Landroid/os/Bundle;-><init>()V

    iput-object v0, p0, La/q8;->f:Landroid/os/Bundle;

    iput-object p1, p0, La/q8;->b:La/p8$d;

    sget v0, Landroid/os/Build$VERSION;->SDK_INT:I

    const/16 v3, 0x1a

    if-lt v0, v3, :cond_0

    new-instance v2, Landroid/app/Notification$Builder;

    iget-object v1, p1, La/p8$d;->a:Landroid/content/Context;

    iget-object v0, p1, La/p8$d;->I:Ljava/lang/String;

    invoke-direct {v2, v1, v0}, Landroid/app/Notification$Builder;-><init>(Landroid/content/Context;Ljava/lang/String;)V

    goto :goto_0

    :cond_0
    new-instance v2, Landroid/app/Notification$Builder;

    iget-object v0, p1, La/p8$d;->a:Landroid/content/Context;

    invoke-direct {v2, v0}, Landroid/app/Notification$Builder;-><init>(Landroid/content/Context;)V

    :goto_0
    iput-object v2, p0, La/q8;->a:Landroid/app/Notification$Builder;

    iget-object v5, p1, La/p8$d;->N:Landroid/app/Notification;

    iget-object v2, p0, La/q8;->a:Landroid/app/Notification$Builder;

    iget-wide v0, v5, Landroid/app/Notification;->when:J

    invoke-virtual {v2, v0, v1}, Landroid/app/Notification$Builder;->setWhen(J)Landroid/app/Notification$Builder;

    move-result-object v2

    iget v1, v5, Landroid/app/Notification;->icon:I

    iget v0, v5, Landroid/app/Notification;->iconLevel:I

    invoke-virtual {v2, v1, v0}, Landroid/app/Notification$Builder;->setSmallIcon(II)Landroid/app/Notification$Builder;

    move-result-object v1

    iget-object v0, v5, Landroid/app/Notification;->contentView:Landroid/widget/RemoteViews;

    invoke-virtual {v1, v0}, Landroid/app/Notification$Builder;->setContent(Landroid/widget/RemoteViews;)Landroid/app/Notification$Builder;

    move-result-object v2

    iget-object v1, v5, Landroid/app/Notification;->tickerText:Ljava/lang/CharSequence;

    iget-object v0, p1, La/p8$d;->h:Landroid/widget/RemoteViews;

    invoke-virtual {v2, v1, v0}, Landroid/app/Notification$Builder;->setTicker(Ljava/lang/CharSequence;Landroid/widget/RemoteViews;)Landroid/app/Notification$Builder;

    move-result-object v1

    iget-object v0, v5, Landroid/app/Notification;->vibrate:[J

    invoke-virtual {v1, v0}, Landroid/app/Notification$Builder;->setVibrate([J)Landroid/app/Notification$Builder;

    move-result-object v4

    iget v2, v5, Landroid/app/Notification;->ledARGB:I

    iget v1, v5, Landroid/app/Notification;->ledOnMS:I

    iget v0, v5, Landroid/app/Notification;->ledOffMS:I

    invoke-virtual {v4, v2, v1, v0}, Landroid/app/Notification$Builder;->setLights(III)Landroid/app/Notification$Builder;

    move-result-object v1

    iget v0, v5, Landroid/app/Notification;->flags:I

    and-int/lit8 v0, v0, 0x2

    const/4 v8, 0x1

    const/4 v2, 0x0

    if-eqz v0, :cond_1

    const/4 v0, 0x1

    goto :goto_1

    :cond_1
    const/4 v0, 0x0

    :goto_1
    invoke-virtual {v1, v0}, Landroid/app/Notification$Builder;->setOngoing(Z)Landroid/app/Notification$Builder;

    move-result-object v1

    iget v0, v5, Landroid/app/Notification;->flags:I

    and-int/lit8 v0, v0, 0x8

    if-eqz v0, :cond_2

    const/4 v0, 0x1

    goto :goto_2

    :cond_2
    const/4 v0, 0x0

    :goto_2
    invoke-virtual {v1, v0}, Landroid/app/Notification$Builder;->setOnlyAlertOnce(Z)Landroid/app/Notification$Builder;

    move-result-object v1

    iget v0, v5, Landroid/app/Notification;->flags:I

    const/16 v9, 0x10

    and-int/2addr v0, v9

    if-eqz v0, :cond_3

    const/4 v0, 0x1

    goto :goto_3

    :cond_3
    const/4 v0, 0x0

    :goto_3
    invoke-virtual {v1, v0}, Landroid/app/Notification$Builder;->setAutoCancel(Z)Landroid/app/Notification$Builder;

    move-result-object v1

    iget v0, v5, Landroid/app/Notification;->defaults:I

    invoke-virtual {v1, v0}, Landroid/app/Notification$Builder;->setDefaults(I)Landroid/app/Notification$Builder;

    move-result-object v1

    iget-object v0, p1, La/p8$d;->d:Ljava/lang/CharSequence;

    invoke-virtual {v1, v0}, Landroid/app/Notification$Builder;->setContentTitle(Ljava/lang/CharSequence;)Landroid/app/Notification$Builder;

    move-result-object v1

    iget-object v0, p1, La/p8$d;->e:Ljava/lang/CharSequence;

    invoke-virtual {v1, v0}, Landroid/app/Notification$Builder;->setContentText(Ljava/lang/CharSequence;)Landroid/app/Notification$Builder;

    move-result-object v1

    iget-object v0, p1, La/p8$d;->j:Ljava/lang/CharSequence;

    invoke-virtual {v1, v0}, Landroid/app/Notification$Builder;->setContentInfo(Ljava/lang/CharSequence;)Landroid/app/Notification$Builder;

    move-result-object v1

    iget-object v0, p1, La/p8$d;->f:Landroid/app/PendingIntent;

    invoke-virtual {v1, v0}, Landroid/app/Notification$Builder;->setContentIntent(Landroid/app/PendingIntent;)Landroid/app/Notification$Builder;

    move-result-object v1

    iget-object v0, v5, Landroid/app/Notification;->deleteIntent:Landroid/app/PendingIntent;

    invoke-virtual {v1, v0}, Landroid/app/Notification$Builder;->setDeleteIntent(Landroid/app/PendingIntent;)Landroid/app/Notification$Builder;

    move-result-object v4

    iget-object v1, p1, La/p8$d;->g:Landroid/app/PendingIntent;

    iget v0, v5, Landroid/app/Notification;->flags:I

    and-int/lit16 v0, v0, 0x80

    if-eqz v0, :cond_4

    const/4 v0, 0x1

    goto :goto_4

    :cond_4
    const/4 v0, 0x0

    :goto_4
    invoke-virtual {v4, v1, v0}, Landroid/app/Notification$Builder;->setFullScreenIntent(Landroid/app/PendingIntent;Z)Landroid/app/Notification$Builder;

    move-result-object v1

    iget-object v0, p1, La/p8$d;->i:Landroid/graphics/Bitmap;

    invoke-virtual {v1, v0}, Landroid/app/Notification$Builder;->setLargeIcon(Landroid/graphics/Bitmap;)Landroid/app/Notification$Builder;

    move-result-object v1

    iget v0, p1, La/p8$d;->k:I

    invoke-virtual {v1, v0}, Landroid/app/Notification$Builder;->setNumber(I)Landroid/app/Notification$Builder;

    move-result-object v6

    iget v4, p1, La/p8$d;->r:I

    iget v1, p1, La/p8$d;->s:I

    iget-boolean v0, p1, La/p8$d;->t:Z

    invoke-virtual {v6, v4, v1, v0}, Landroid/app/Notification$Builder;->setProgress(IIZ)Landroid/app/Notification$Builder;

    sget v0, Landroid/os/Build$VERSION;->SDK_INT:I

    const/16 v4, 0x15

    if-ge v0, v4, :cond_5

    iget-object v6, p0, La/q8;->a:Landroid/app/Notification$Builder;

    iget-object v1, v5, Landroid/app/Notification;->sound:Landroid/net/Uri;

    iget v0, v5, Landroid/app/Notification;->audioStreamType:I

    invoke-virtual {v6, v1, v0}, Landroid/app/Notification$Builder;->setSound(Landroid/net/Uri;I)Landroid/app/Notification$Builder;

    :cond_5
    sget v0, Landroid/os/Build$VERSION;->SDK_INT:I

    const/16 v7, 0x14

    if-lt v0, v9, :cond_c

    iget-object v1, p0, La/q8;->a:Landroid/app/Notification$Builder;

    iget-object v0, p1, La/p8$d;->p:Ljava/lang/CharSequence;

    invoke-virtual {v1, v0}, Landroid/app/Notification$Builder;->setSubText(Ljava/lang/CharSequence;)Landroid/app/Notification$Builder;

    move-result-object v1

    iget-boolean v0, p1, La/p8$d;->n:Z

    invoke-virtual {v1, v0}, Landroid/app/Notification$Builder;->setUsesChronometer(Z)Landroid/app/Notification$Builder;

    move-result-object v1

    iget v0, p1, La/p8$d;->l:I

    invoke-virtual {v1, v0}, Landroid/app/Notification$Builder;->setPriority(I)Landroid/app/Notification$Builder;

    iget-object v0, p1, La/p8$d;->b:Ljava/util/ArrayList;

    invoke-virtual {v0}, Ljava/util/ArrayList;->iterator()Ljava/util/Iterator;

    move-result-object v1

    :goto_5
    invoke-interface {v1}, Ljava/util/Iterator;->hasNext()Z

    move-result v0

    if-eqz v0, :cond_6

    invoke-interface {v1}, Ljava/util/Iterator;->next()Ljava/lang/Object;

    move-result-object v0

    check-cast v0, La/p8$a;

    invoke-virtual {p0, v0}, La/q8;->a(La/p8$a;)V

    goto :goto_5

    :cond_6
    iget-object v1, p1, La/p8$d;->B:Landroid/os/Bundle;

    if-eqz v1, :cond_7

    iget-object v0, p0, La/q8;->f:Landroid/os/Bundle;

    invoke-virtual {v0, v1}, Landroid/os/Bundle;->putAll(Landroid/os/Bundle;)V

    :cond_7
    sget v0, Landroid/os/Build$VERSION;->SDK_INT:I

    if-ge v0, v7, :cond_b

    iget-boolean v0, p1, La/p8$d;->x:Z

    if-eqz v0, :cond_8

    iget-object v1, p0, La/q8;->f:Landroid/os/Bundle;

    const-string v0, "android.support.localOnly"

    invoke-virtual {v1, v0, v8}, Landroid/os/Bundle;->putBoolean(Ljava/lang/String;Z)V

    :cond_8
    iget-object v6, p1, La/p8$d;->u:Ljava/lang/String;

    if-eqz v6, :cond_a

    iget-object v1, p0, La/q8;->f:Landroid/os/Bundle;

    const-string v0, "android.support.groupKey"

    invoke-virtual {v1, v0, v6}, Landroid/os/Bundle;->putString(Ljava/lang/String;Ljava/lang/String;)V

    iget-boolean v0, p1, La/p8$d;->v:Z

    if-eqz v0, :cond_9

    iget-object v1, p0, La/q8;->f:Landroid/os/Bundle;

    const-string v0, "android.support.isGroupSummary"

    goto :goto_6

    :cond_9
    iget-object v1, p0, La/q8;->f:Landroid/os/Bundle;

    const-string v0, "android.support.useSideChannel"

    :goto_6
    invoke-virtual {v1, v0, v8}, Landroid/os/Bundle;->putBoolean(Ljava/lang/String;Z)V

    :cond_a
    iget-object v6, p1, La/p8$d;->w:Ljava/lang/String;

    if-eqz v6, :cond_b

    iget-object v1, p0, La/q8;->f:Landroid/os/Bundle;

    const-string v0, "android.support.sortKey"

    invoke-virtual {v1, v0, v6}, Landroid/os/Bundle;->putString(Ljava/lang/String;Ljava/lang/String;)V

    :cond_b
    iget-object v0, p1, La/p8$d;->F:Landroid/widget/RemoteViews;

    iput-object v0, p0, La/q8;->c:Landroid/widget/RemoteViews;

    iget-object v0, p1, La/p8$d;->G:Landroid/widget/RemoteViews;

    iput-object v0, p0, La/q8;->d:Landroid/widget/RemoteViews;

    :cond_c
    sget v1, Landroid/os/Build$VERSION;->SDK_INT:I

    const/16 v0, 0x13

    if-lt v1, v0, :cond_d

    iget-object v1, p0, La/q8;->a:Landroid/app/Notification$Builder;

    iget-boolean v0, p1, La/p8$d;->m:Z

    invoke-virtual {v1, v0}, Landroid/app/Notification$Builder;->setShowWhen(Z)Landroid/app/Notification$Builder;

    sget v0, Landroid/os/Build$VERSION;->SDK_INT:I

    if-ge v0, v4, :cond_d

    iget-object v0, p1, La/p8$d;->O:Ljava/util/ArrayList;

    if-eqz v0, :cond_d

    invoke-virtual {v0}, Ljava/util/ArrayList;->isEmpty()Z

    move-result v0

    if-nez v0, :cond_d

    iget-object v6, p0, La/q8;->f:Landroid/os/Bundle;

    iget-object v1, p1, La/p8$d;->O:Ljava/util/ArrayList;

    invoke-virtual {v1}, Ljava/util/ArrayList;->size()I

    move-result v0

    new-array v0, v0, [Ljava/lang/String;

    invoke-virtual {v1, v0}, Ljava/util/ArrayList;->toArray([Ljava/lang/Object;)[Ljava/lang/Object;

    move-result-object v1

    check-cast v1, [Ljava/lang/String;

    const-string v0, "android.people"

    invoke-virtual {v6, v0, v1}, Landroid/os/Bundle;->putStringArray(Ljava/lang/String;[Ljava/lang/String;)V

    :cond_d
    sget v0, Landroid/os/Build$VERSION;->SDK_INT:I

    if-lt v0, v7, :cond_e

    iget-object v1, p0, La/q8;->a:Landroid/app/Notification$Builder;

    iget-boolean v0, p1, La/p8$d;->x:Z

    invoke-virtual {v1, v0}, Landroid/app/Notification$Builder;->setLocalOnly(Z)Landroid/app/Notification$Builder;

    move-result-object v1

    iget-object v0, p1, La/p8$d;->u:Ljava/lang/String;

    invoke-virtual {v1, v0}, Landroid/app/Notification$Builder;->setGroup(Ljava/lang/String;)Landroid/app/Notification$Builder;

    move-result-object v1

    iget-boolean v0, p1, La/p8$d;->v:Z

    invoke-virtual {v1, v0}, Landroid/app/Notification$Builder;->setGroupSummary(Z)Landroid/app/Notification$Builder;

    move-result-object v1

    iget-object v0, p1, La/p8$d;->w:Ljava/lang/String;

    invoke-virtual {v1, v0}, Landroid/app/Notification$Builder;->setSortKey(Ljava/lang/String;)Landroid/app/Notification$Builder;

    iget v0, p1, La/p8$d;->M:I

    iput v0, p0, La/q8;->g:I

    :cond_e
    sget v0, Landroid/os/Build$VERSION;->SDK_INT:I

    if-lt v0, v4, :cond_12

    iget-object v1, p0, La/q8;->a:Landroid/app/Notification$Builder;

    iget-object v0, p1, La/p8$d;->A:Ljava/lang/String;

    invoke-virtual {v1, v0}, Landroid/app/Notification$Builder;->setCategory(Ljava/lang/String;)Landroid/app/Notification$Builder;

    move-result-object v1

    iget v0, p1, La/p8$d;->C:I

    invoke-virtual {v1, v0}, Landroid/app/Notification$Builder;->setColor(I)Landroid/app/Notification$Builder;

    move-result-object v1

    iget v0, p1, La/p8$d;->D:I

    invoke-virtual {v1, v0}, Landroid/app/Notification$Builder;->setVisibility(I)Landroid/app/Notification$Builder;

    move-result-object v1

    iget-object v0, p1, La/p8$d;->E:Landroid/app/Notification;

    invoke-virtual {v1, v0}, Landroid/app/Notification$Builder;->setPublicVersion(Landroid/app/Notification;)Landroid/app/Notification$Builder;

    move-result-object v4

    iget-object v1, v5, Landroid/app/Notification;->sound:Landroid/net/Uri;

    iget-object v0, v5, Landroid/app/Notification;->audioAttributes:Landroid/media/AudioAttributes;

    invoke-virtual {v4, v1, v0}, Landroid/app/Notification$Builder;->setSound(Landroid/net/Uri;Landroid/media/AudioAttributes;)Landroid/app/Notification$Builder;

    iget-object v0, p1, La/p8$d;->O:Ljava/util/ArrayList;

    invoke-virtual {v0}, Ljava/util/ArrayList;->iterator()Ljava/util/Iterator;

    move-result-object v4

    :goto_7
    invoke-interface {v4}, Ljava/util/Iterator;->hasNext()Z

    move-result v0

    if-eqz v0, :cond_f

    invoke-interface {v4}, Ljava/util/Iterator;->next()Ljava/lang/Object;

    move-result-object v1

    check-cast v1, Ljava/lang/String;

    iget-object v0, p0, La/q8;->a:Landroid/app/Notification$Builder;

    invoke-virtual {v0, v1}, Landroid/app/Notification$Builder;->addPerson(Ljava/lang/String;)Landroid/app/Notification$Builder;

    goto :goto_7

    :cond_f
    iget-object v0, p1, La/p8$d;->H:Landroid/widget/RemoteViews;

    iput-object v0, p0, La/q8;->h:Landroid/widget/RemoteViews;

    iget-object v0, p1, La/p8$d;->c:Ljava/util/ArrayList;

    invoke-virtual {v0}, Ljava/util/ArrayList;->size()I

    move-result v0

    if-lez v0, :cond_12

    invoke-virtual {p1}, La/p8$d;->b()Landroid/os/Bundle;

    move-result-object v0

    const-string v6, "android.car.EXTENSIONS"

    invoke-virtual {v0, v6}, Landroid/os/Bundle;->getBundle(Ljava/lang/String;)Landroid/os/Bundle;

    move-result-object v5

    if-nez v5, :cond_10

    new-instance v5, Landroid/os/Bundle;

    invoke-direct {v5}, Landroid/os/Bundle;-><init>()V

    :cond_10
    new-instance v7, Landroid/os/Bundle;

    invoke-direct {v7}, Landroid/os/Bundle;-><init>()V

    const/4 v4, 0x0

    :goto_8
    iget-object v0, p1, La/p8$d;->c:Ljava/util/ArrayList;

    invoke-virtual {v0}, Ljava/util/ArrayList;->size()I

    move-result v0

    if-ge v4, v0, :cond_11

    invoke-static {v4}, Ljava/lang/Integer;->toString(I)Ljava/lang/String;

    move-result-object v1

    iget-object v0, p1, La/p8$d;->c:Ljava/util/ArrayList;

    invoke-virtual {v0, v4}, Ljava/util/ArrayList;->get(I)Ljava/lang/Object;

    move-result-object v0

    check-cast v0, La/p8$a;

    invoke-static {v0}, La/r8;->a(La/p8$a;)Landroid/os/Bundle;

    move-result-object v0

    invoke-virtual {v7, v1, v0}, Landroid/os/Bundle;->putBundle(Ljava/lang/String;Landroid/os/Bundle;)V

    add-int/lit8 v4, v4, 0x1

    goto :goto_8

    :cond_11
    const-string v0, "invisible_actions"

    invoke-virtual {v5, v0, v7}, Landroid/os/Bundle;->putBundle(Ljava/lang/String;Landroid/os/Bundle;)V

    invoke-virtual {p1}, La/p8$d;->b()Landroid/os/Bundle;

    move-result-object v0

    invoke-virtual {v0, v6, v5}, Landroid/os/Bundle;->putBundle(Ljava/lang/String;Landroid/os/Bundle;)V

    iget-object v0, p0, La/q8;->f:Landroid/os/Bundle;

    invoke-virtual {v0, v6, v5}, Landroid/os/Bundle;->putBundle(Ljava/lang/String;Landroid/os/Bundle;)V

    :cond_12
    sget v1, Landroid/os/Build$VERSION;->SDK_INT:I

    const/16 v0, 0x18

    if-lt v1, v0, :cond_15

    iget-object v1, p0, La/q8;->a:Landroid/app/Notification$Builder;

    iget-object v0, p1, La/p8$d;->B:Landroid/os/Bundle;

    invoke-virtual {v1, v0}, Landroid/app/Notification$Builder;->setExtras(Landroid/os/Bundle;)Landroid/app/Notification$Builder;

    move-result-object v1

    iget-object v0, p1, La/p8$d;->q:[Ljava/lang/CharSequence;

    invoke-virtual {v1, v0}, Landroid/app/Notification$Builder;->setRemoteInputHistory([Ljava/lang/CharSequence;)Landroid/app/Notification$Builder;

    iget-object v1, p1, La/p8$d;->F:Landroid/widget/RemoteViews;

    if-eqz v1, :cond_13

    iget-object v0, p0, La/q8;->a:Landroid/app/Notification$Builder;

    invoke-virtual {v0, v1}, Landroid/app/Notification$Builder;->setCustomContentView(Landroid/widget/RemoteViews;)Landroid/app/Notification$Builder;

    :cond_13
    iget-object v1, p1, La/p8$d;->G:Landroid/widget/RemoteViews;

    if-eqz v1, :cond_14

    iget-object v0, p0, La/q8;->a:Landroid/app/Notification$Builder;

    invoke-virtual {v0, v1}, Landroid/app/Notification$Builder;->setCustomBigContentView(Landroid/widget/RemoteViews;)Landroid/app/Notification$Builder;

    :cond_14
    iget-object v1, p1, La/p8$d;->H:Landroid/widget/RemoteViews;

    if-eqz v1, :cond_15

    iget-object v0, p0, La/q8;->a:Landroid/app/Notification$Builder;

    invoke-virtual {v0, v1}, Landroid/app/Notification$Builder;->setCustomHeadsUpContentView(Landroid/widget/RemoteViews;)Landroid/app/Notification$Builder;

    :cond_15
    sget v0, Landroid/os/Build$VERSION;->SDK_INT:I

    if-lt v0, v3, :cond_17

    iget-object v1, p0, La/q8;->a:Landroid/app/Notification$Builder;

    iget v0, p1, La/p8$d;->J:I

    invoke-virtual {v1, v0}, Landroid/app/Notification$Builder;->setBadgeIconType(I)Landroid/app/Notification$Builder;

    move-result-object v1

    iget-object v0, p1, La/p8$d;->K:Ljava/lang/String;

    invoke-virtual {v1, v0}, Landroid/app/Notification$Builder;->setShortcutId(Ljava/lang/String;)Landroid/app/Notification$Builder;

    move-result-object v3

    iget-wide v0, p1, La/p8$d;->L:J

    invoke-virtual {v3, v0, v1}, Landroid/app/Notification$Builder;->setTimeoutAfter(J)Landroid/app/Notification$Builder;

    move-result-object v1

    iget v0, p1, La/p8$d;->M:I

    invoke-virtual {v1, v0}, Landroid/app/Notification$Builder;->setGroupAlertBehavior(I)Landroid/app/Notification$Builder;

    iget-boolean v0, p1, La/p8$d;->z:Z

    if-eqz v0, :cond_16

    iget-object v1, p0, La/q8;->a:Landroid/app/Notification$Builder;

    iget-boolean v0, p1, La/p8$d;->y:Z

    invoke-virtual {v1, v0}, Landroid/app/Notification$Builder;->setColorized(Z)Landroid/app/Notification$Builder;

    :cond_16
    iget-object v0, p1, La/p8$d;->I:Ljava/lang/String;

    invoke-static {v0}, Landroid/text/TextUtils;->isEmpty(Ljava/lang/CharSequence;)Z

    move-result v0

    if-nez v0, :cond_17

    iget-object v0, p0, La/q8;->a:Landroid/app/Notification$Builder;

    const/4 v1, 0x0

    invoke-virtual {v0, v1}, Landroid/app/Notification$Builder;->setSound(Landroid/net/Uri;)Landroid/app/Notification$Builder;

    move-result-object v0

    invoke-virtual {v0, v2}, Landroid/app/Notification$Builder;->setDefaults(I)Landroid/app/Notification$Builder;

    move-result-object v0

    invoke-virtual {v0, v2, v2, v2}, Landroid/app/Notification$Builder;->setLights(III)Landroid/app/Notification$Builder;

    move-result-object v0

    invoke-virtual {v0, v1}, Landroid/app/Notification$Builder;->setVibrate([J)Landroid/app/Notification$Builder;

    :cond_17
    return-void
.end method


# virtual methods
.method public a()Landroid/app/Notification$Builder;
    .locals 0

    iget-object p0, p0, La/q8;->a:Landroid/app/Notification$Builder;

    return-object p0
.end method

.method public final a(La/p8$a;)V
    .locals 5

    sget v1, Landroid/os/Build$VERSION;->SDK_INT:I

    const/16 v0, 0x14

    if-lt v1, v0, :cond_4

    new-instance v2, Landroid/app/Notification$Action$Builder;

    invoke-virtual {p1}, La/p8$a;->e()I

    move-result v3

    invoke-virtual {p1}, La/p8$a;->i()Ljava/lang/CharSequence;

    move-result-object v1

    invoke-virtual {p1}, La/p8$a;->a()Landroid/app/PendingIntent;

    move-result-object v0

    invoke-direct {v2, v3, v1, v0}, Landroid/app/Notification$Action$Builder;-><init>(ILjava/lang/CharSequence;Landroid/app/PendingIntent;)V

    invoke-virtual {p1}, La/p8$a;->f()[La/t8;

    move-result-object v0

    if-eqz v0, :cond_0

    invoke-virtual {p1}, La/p8$a;->f()[La/t8;

    move-result-object v0

    invoke-static {v0}, La/t8;->a([La/t8;)[Landroid/app/RemoteInput;

    move-result-object v4

    array-length v3, v4

    const/4 v1, 0x0

    :goto_0
    if-ge v1, v3, :cond_0

    aget-object v0, v4, v1

    invoke-virtual {v2, v0}, Landroid/app/Notification$Action$Builder;->addRemoteInput(Landroid/app/RemoteInput;)Landroid/app/Notification$Action$Builder;

    add-int/lit8 v1, v1, 0x1

    goto :goto_0

    :cond_0
    invoke-virtual {p1}, La/p8$a;->d()Landroid/os/Bundle;

    move-result-object v0

    if-eqz v0, :cond_1

    new-instance v3, Landroid/os/Bundle;

    invoke-virtual {p1}, La/p8$a;->d()Landroid/os/Bundle;

    move-result-object v0

    invoke-direct {v3, v0}, Landroid/os/Bundle;-><init>(Landroid/os/Bundle;)V

    goto :goto_1

    :cond_1
    new-instance v3, Landroid/os/Bundle;

    invoke-direct {v3}, Landroid/os/Bundle;-><init>()V

    :goto_1
    invoke-virtual {p1}, La/p8$a;->b()Z

    move-result v1

    const-string v0, "android.support.allowGeneratedReplies"

    invoke-virtual {v3, v0, v1}, Landroid/os/Bundle;->putBoolean(Ljava/lang/String;Z)V

    sget v1, Landroid/os/Build$VERSION;->SDK_INT:I

    const/16 v0, 0x18

    if-lt v1, v0, :cond_2

    invoke-virtual {p1}, La/p8$a;->b()Z

    move-result v0

    invoke-virtual {v2, v0}, Landroid/app/Notification$Action$Builder;->setAllowGeneratedReplies(Z)Landroid/app/Notification$Action$Builder;

    :cond_2
    invoke-virtual {p1}, La/p8$a;->g()I

    move-result v1

    const-string v0, "android.support.action.semanticAction"

    invoke-virtual {v3, v0, v1}, Landroid/os/Bundle;->putInt(Ljava/lang/String;I)V

    sget v1, Landroid/os/Build$VERSION;->SDK_INT:I

    const/16 v0, 0x1c

    if-lt v1, v0, :cond_3

    invoke-virtual {p1}, La/p8$a;->g()I

    move-result v0

    invoke-virtual {v2, v0}, Landroid/app/Notification$Action$Builder;->setSemanticAction(I)Landroid/app/Notification$Action$Builder;

    :cond_3
    invoke-virtual {p1}, La/p8$a;->h()Z

    move-result v1

    const-string v0, "android.support.action.showsUserInterface"

    invoke-virtual {v3, v0, v1}, Landroid/os/Bundle;->putBoolean(Ljava/lang/String;Z)V

    invoke-virtual {v2, v3}, Landroid/app/Notification$Action$Builder;->addExtras(Landroid/os/Bundle;)Landroid/app/Notification$Action$Builder;

    iget-object v1, p0, La/q8;->a:Landroid/app/Notification$Builder;

    invoke-virtual {v2}, Landroid/app/Notification$Action$Builder;->build()Landroid/app/Notification$Action;

    move-result-object v0

    invoke-virtual {v1, v0}, Landroid/app/Notification$Builder;->addAction(Landroid/app/Notification$Action;)Landroid/app/Notification$Builder;

    goto :goto_2

    :cond_4
    const/16 v0, 0x10

    if-lt v1, v0, :cond_5

    iget-object v1, p0, La/q8;->e:Ljava/util/List;

    iget-object v0, p0, La/q8;->a:Landroid/app/Notification$Builder;

    invoke-static {v0, p1}, La/r8;->a(Landroid/app/Notification$Builder;La/p8$a;)Landroid/os/Bundle;

    move-result-object v0

    invoke-interface {v1, v0}, Ljava/util/List;->add(Ljava/lang/Object;)Z

    :cond_5
    :goto_2
    return-void
.end method

.method public final a(Landroid/app/Notification;)V
    .locals 0

    const/4 p0, 0x0

    iput-object p0, p1, Landroid/app/Notification;->sound:Landroid/net/Uri;

    iput-object p0, p1, Landroid/app/Notification;->vibrate:[J

    iget p0, p1, Landroid/app/Notification;->defaults:I

    and-int/lit8 p0, p0, -0x2

    iput p0, p1, Landroid/app/Notification;->defaults:I

    iget p0, p1, Landroid/app/Notification;->defaults:I

    and-int/lit8 p0, p0, -0x3

    iput p0, p1, Landroid/app/Notification;->defaults:I

    return-void
.end method

.method public b()Landroid/app/Notification;
    .locals 5

    iget-object v0, p0, La/q8;->b:La/p8$d;

    iget-object v4, v0, La/p8$d;->o:La/p8$e;

    if-eqz v4, :cond_0

    invoke-virtual {v4, p0}, La/p8$e;->a(La/q8;)V

    :cond_0
    if-eqz v4, :cond_1

    invoke-virtual {v4, p0}, La/p8$e;->c(La/q8;)Landroid/widget/RemoteViews;

    move-result-object v0

    goto :goto_0

    :cond_1
    const/4 v0, 0x0

    :goto_0
    invoke-virtual {p0}, La/q8;->c()Landroid/app/Notification;

    move-result-object v3

    if-eqz v0, :cond_2

    :goto_1
    iput-object v0, v3, Landroid/app/Notification;->contentView:Landroid/widget/RemoteViews;

    goto :goto_2

    :cond_2
    iget-object v0, p0, La/q8;->b:La/p8$d;

    iget-object v0, v0, La/p8$d;->F:Landroid/widget/RemoteViews;

    if-eqz v0, :cond_3

    goto :goto_1

    :cond_3
    :goto_2
    sget v0, Landroid/os/Build$VERSION;->SDK_INT:I

    const/16 v2, 0x10

    if-lt v0, v2, :cond_4

    if-eqz v4, :cond_4

    invoke-virtual {v4, p0}, La/p8$e;->b(La/q8;)Landroid/widget/RemoteViews;

    move-result-object v0

    if-eqz v0, :cond_4

    iput-object v0, v3, Landroid/app/Notification;->bigContentView:Landroid/widget/RemoteViews;

    :cond_4
    sget v1, Landroid/os/Build$VERSION;->SDK_INT:I

    const/16 v0, 0x15

    if-lt v1, v0, :cond_5

    if-eqz v4, :cond_5

    iget-object v0, p0, La/q8;->b:La/p8$d;

    iget-object v0, v0, La/p8$d;->o:La/p8$e;

    invoke-virtual {v0, p0}, La/p8$e;->d(La/q8;)Landroid/widget/RemoteViews;

    move-result-object v0

    if-eqz v0, :cond_5

    iput-object v0, v3, Landroid/app/Notification;->headsUpContentView:Landroid/widget/RemoteViews;

    :cond_5
    sget v0, Landroid/os/Build$VERSION;->SDK_INT:I

    if-lt v0, v2, :cond_6

    if-eqz v4, :cond_6

    invoke-static {v3}, La/p8;->a(Landroid/app/Notification;)Landroid/os/Bundle;

    move-result-object v0

    if-eqz v0, :cond_6

    invoke-virtual {v4, v0}, La/p8$e;->a(Landroid/os/Bundle;)V

    :cond_6
    return-object v3
.end method

.method public c()Landroid/app/Notification;
    .locals 7

    sget v1, Landroid/os/Build$VERSION;->SDK_INT:I

    const/16 v0, 0x1a

    if-lt v1, v0, :cond_0

    iget-object v0, p0, La/q8;->a:Landroid/app/Notification$Builder;

    invoke-virtual {v0}, Landroid/app/Notification$Builder;->build()Landroid/app/Notification;

    move-result-object v0

    return-object v0

    :cond_0
    const/16 v0, 0x18

    const/4 v2, 0x1

    const/4 v3, 0x2

    if-lt v1, v0, :cond_3

    iget-object v0, p0, La/q8;->a:Landroid/app/Notification$Builder;

    invoke-virtual {v0}, Landroid/app/Notification$Builder;->build()Landroid/app/Notification;

    move-result-object v1

    iget v0, p0, La/q8;->g:I

    if-eqz v0, :cond_2

    invoke-virtual {v1}, Landroid/app/Notification;->getGroup()Ljava/lang/String;

    move-result-object v0

    if-eqz v0, :cond_1

    iget v0, v1, Landroid/app/Notification;->flags:I

    and-int/lit16 v0, v0, 0x200

    if-eqz v0, :cond_1

    iget v0, p0, La/q8;->g:I

    if-ne v0, v3, :cond_1

    invoke-virtual {p0, v1}, La/q8;->a(Landroid/app/Notification;)V

    :cond_1
    invoke-virtual {v1}, Landroid/app/Notification;->getGroup()Ljava/lang/String;

    move-result-object v0

    if-eqz v0, :cond_2

    iget v0, v1, Landroid/app/Notification;->flags:I

    and-int/lit16 v0, v0, 0x200

    if-nez v0, :cond_2

    iget v0, p0, La/q8;->g:I

    if-ne v0, v2, :cond_2

    invoke-virtual {p0, v1}, La/q8;->a(Landroid/app/Notification;)V

    :cond_2
    return-object v1

    :cond_3
    const/16 v0, 0x15

    if-lt v1, v0, :cond_9

    iget-object v1, p0, La/q8;->a:Landroid/app/Notification$Builder;

    iget-object v0, p0, La/q8;->f:Landroid/os/Bundle;

    invoke-virtual {v1, v0}, Landroid/app/Notification$Builder;->setExtras(Landroid/os/Bundle;)Landroid/app/Notification$Builder;

    iget-object v0, p0, La/q8;->a:Landroid/app/Notification$Builder;

    invoke-virtual {v0}, Landroid/app/Notification$Builder;->build()Landroid/app/Notification;

    move-result-object v1

    iget-object v0, p0, La/q8;->c:Landroid/widget/RemoteViews;

    if-eqz v0, :cond_4

    iput-object v0, v1, Landroid/app/Notification;->contentView:Landroid/widget/RemoteViews;

    :cond_4
    iget-object v0, p0, La/q8;->d:Landroid/widget/RemoteViews;

    if-eqz v0, :cond_5

    iput-object v0, v1, Landroid/app/Notification;->bigContentView:Landroid/widget/RemoteViews;

    :cond_5
    iget-object v0, p0, La/q8;->h:Landroid/widget/RemoteViews;

    if-eqz v0, :cond_6

    iput-object v0, v1, Landroid/app/Notification;->headsUpContentView:Landroid/widget/RemoteViews;

    :cond_6
    iget v0, p0, La/q8;->g:I

    if-eqz v0, :cond_8

    invoke-virtual {v1}, Landroid/app/Notification;->getGroup()Ljava/lang/String;

    move-result-object v0

    if-eqz v0, :cond_7

    iget v0, v1, Landroid/app/Notification;->flags:I

    and-int/lit16 v0, v0, 0x200

    if-eqz v0, :cond_7

    iget v0, p0, La/q8;->g:I

    if-ne v0, v3, :cond_7

    invoke-virtual {p0, v1}, La/q8;->a(Landroid/app/Notification;)V

    :cond_7
    invoke-virtual {v1}, Landroid/app/Notification;->getGroup()Ljava/lang/String;

    move-result-object v0

    if-eqz v0, :cond_8

    iget v0, v1, Landroid/app/Notification;->flags:I

    and-int/lit16 v0, v0, 0x200

    if-nez v0, :cond_8

    iget v0, p0, La/q8;->g:I

    if-ne v0, v2, :cond_8

    invoke-virtual {p0, v1}, La/q8;->a(Landroid/app/Notification;)V

    :cond_8
    return-object v1

    :cond_9
    const/16 v0, 0x14

    if-lt v1, v0, :cond_e

    iget-object v1, p0, La/q8;->a:Landroid/app/Notification$Builder;

    iget-object v0, p0, La/q8;->f:Landroid/os/Bundle;

    invoke-virtual {v1, v0}, Landroid/app/Notification$Builder;->setExtras(Landroid/os/Bundle;)Landroid/app/Notification$Builder;

    iget-object v0, p0, La/q8;->a:Landroid/app/Notification$Builder;

    invoke-virtual {v0}, Landroid/app/Notification$Builder;->build()Landroid/app/Notification;

    move-result-object v1

    iget-object v0, p0, La/q8;->c:Landroid/widget/RemoteViews;

    if-eqz v0, :cond_a

    iput-object v0, v1, Landroid/app/Notification;->contentView:Landroid/widget/RemoteViews;

    :cond_a
    iget-object v0, p0, La/q8;->d:Landroid/widget/RemoteViews;

    if-eqz v0, :cond_b

    iput-object v0, v1, Landroid/app/Notification;->bigContentView:Landroid/widget/RemoteViews;

    :cond_b
    iget v0, p0, La/q8;->g:I

    if-eqz v0, :cond_d

    invoke-virtual {v1}, Landroid/app/Notification;->getGroup()Ljava/lang/String;

    move-result-object v0

    if-eqz v0, :cond_c

    iget v0, v1, Landroid/app/Notification;->flags:I

    and-int/lit16 v0, v0, 0x200

    if-eqz v0, :cond_c

    iget v0, p0, La/q8;->g:I

    if-ne v0, v3, :cond_c

    invoke-virtual {p0, v1}, La/q8;->a(Landroid/app/Notification;)V

    :cond_c
    invoke-virtual {v1}, Landroid/app/Notification;->getGroup()Ljava/lang/String;

    move-result-object v0

    if-eqz v0, :cond_d

    iget v0, v1, Landroid/app/Notification;->flags:I

    and-int/lit16 v0, v0, 0x200

    if-nez v0, :cond_d

    iget v0, p0, La/q8;->g:I

    if-ne v0, v2, :cond_d

    invoke-virtual {p0, v1}, La/q8;->a(Landroid/app/Notification;)V

    :cond_d
    return-object v1

    :cond_e
    const/16 v0, 0x13

    const-string v6, "android.support.actionExtras"

    if-lt v1, v0, :cond_12

    iget-object v0, p0, La/q8;->e:Ljava/util/List;

    invoke-static {v0}, La/r8;->a(Ljava/util/List;)Landroid/util/SparseArray;

    move-result-object v1

    if-eqz v1, :cond_f

    iget-object v0, p0, La/q8;->f:Landroid/os/Bundle;

    invoke-virtual {v0, v6, v1}, Landroid/os/Bundle;->putSparseParcelableArray(Ljava/lang/String;Landroid/util/SparseArray;)V

    :cond_f
    iget-object v1, p0, La/q8;->a:Landroid/app/Notification$Builder;

    iget-object v0, p0, La/q8;->f:Landroid/os/Bundle;

    invoke-virtual {v1, v0}, Landroid/app/Notification$Builder;->setExtras(Landroid/os/Bundle;)Landroid/app/Notification$Builder;

    iget-object v0, p0, La/q8;->a:Landroid/app/Notification$Builder;

    invoke-virtual {v0}, Landroid/app/Notification$Builder;->build()Landroid/app/Notification;

    move-result-object v1

    iget-object v0, p0, La/q8;->c:Landroid/widget/RemoteViews;

    if-eqz v0, :cond_10

    iput-object v0, v1, Landroid/app/Notification;->contentView:Landroid/widget/RemoteViews;

    :cond_10
    iget-object v0, p0, La/q8;->d:Landroid/widget/RemoteViews;

    if-eqz v0, :cond_11

    iput-object v0, v1, Landroid/app/Notification;->bigContentView:Landroid/widget/RemoteViews;

    :cond_11
    return-object v1

    :cond_12
    const/16 v0, 0x10

    if-lt v1, v0, :cond_18

    iget-object v0, p0, La/q8;->a:Landroid/app/Notification$Builder;

    invoke-virtual {v0}, Landroid/app/Notification$Builder;->build()Landroid/app/Notification;

    move-result-object v5

    invoke-static {v5}, La/p8;->a(Landroid/app/Notification;)Landroid/os/Bundle;

    move-result-object v4

    new-instance v3, Landroid/os/Bundle;

    iget-object v0, p0, La/q8;->f:Landroid/os/Bundle;

    invoke-direct {v3, v0}, Landroid/os/Bundle;-><init>(Landroid/os/Bundle;)V

    iget-object v0, p0, La/q8;->f:Landroid/os/Bundle;

    invoke-virtual {v0}, Landroid/os/Bundle;->keySet()Ljava/util/Set;

    move-result-object v0

    invoke-interface {v0}, Ljava/util/Set;->iterator()Ljava/util/Iterator;

    move-result-object v2

    :cond_13
    :goto_0
    invoke-interface {v2}, Ljava/util/Iterator;->hasNext()Z

    move-result v0

    if-eqz v0, :cond_14

    invoke-interface {v2}, Ljava/util/Iterator;->next()Ljava/lang/Object;

    move-result-object v1

    check-cast v1, Ljava/lang/String;

    invoke-virtual {v4, v1}, Landroid/os/Bundle;->containsKey(Ljava/lang/String;)Z

    move-result v0

    if-eqz v0, :cond_13

    invoke-virtual {v3, v1}, Landroid/os/Bundle;->remove(Ljava/lang/String;)V

    goto :goto_0

    :cond_14
    invoke-virtual {v4, v3}, Landroid/os/Bundle;->putAll(Landroid/os/Bundle;)V

    iget-object v0, p0, La/q8;->e:Ljava/util/List;

    invoke-static {v0}, La/r8;->a(Ljava/util/List;)Landroid/util/SparseArray;

    move-result-object v1

    if-eqz v1, :cond_15

    invoke-static {v5}, La/p8;->a(Landroid/app/Notification;)Landroid/os/Bundle;

    move-result-object v0

    invoke-virtual {v0, v6, v1}, Landroid/os/Bundle;->putSparseParcelableArray(Ljava/lang/String;Landroid/util/SparseArray;)V

    :cond_15
    iget-object v0, p0, La/q8;->c:Landroid/widget/RemoteViews;

    if-eqz v0, :cond_16

    iput-object v0, v5, Landroid/app/Notification;->contentView:Landroid/widget/RemoteViews;

    :cond_16
    iget-object v0, p0, La/q8;->d:Landroid/widget/RemoteViews;

    if-eqz v0, :cond_17

    iput-object v0, v5, Landroid/app/Notification;->bigContentView:Landroid/widget/RemoteViews;

    :cond_17
    return-object v5

    :cond_18
    iget-object v0, p0, La/q8;->a:Landroid/app/Notification$Builder;

    invoke-virtual {v0}, Landroid/app/Notification$Builder;->getNotification()Landroid/app/Notification;

    move-result-object v0

    return-object v0
.end method
